...
3. **No model yet?** → `stereoframe gen "<prompt>"` writes a textured GLB into `assets/` (real prompt-driven generation when `MESHY_API_KEY` is set — otherwise a sample model) plus a `<name>.gen.json` provenance sidecar. **For better fidelity / reliable orientation, go image-first**: `gen --image front.png` (image-to-3D), `gen --images front,side,back` (multi-view), or `gen "<prompt>" --via-image` (text → OpenAI image → 3D, one command; needs `OPENAI_API_KEY`). For the whole thing in one command, append `--stage <preset> --render` → prompt → model → directed film → mp4.
