...
Stereoframe turns GLBs and HTML markup into deterministic 3D evidence and video. You can inspect/score/compare generated or production GLBs, or describe a three.js scene, camera, lights, and motion with custom elements; the runtime renders every frame as a pure function of seek time; the CLI captures frames in headless Chrome and encodes MP4 via ffmpeg.

## Choosing your path

Pick the lightest path that fits the request, then refine:

1. **Have one or more generated/third-party GLBs and need to know if they are usable?** → `evaluate a.glb b.glb --frames --render` writes per-asset quality reports, heuristic scores, a standardized comparison scene, optional evidence frames, optional MP4, and `REPORT.md`. For a deeper single-asset report, use `evaluate model.glb --audit --frames --render`: it writes `report.html`, `reports/parts.json`, exploded/isolated part evidence and selected-part thumbnail times for separable GLBs, and honest single-mesh limitation warnings. **This is the default for AI-generated GLBs.**
2. **Have an accepted GLB, want a polished preview/explainer fast?** → `stage <model>.glb --preset <name>` auto-frames + finishes any model (presets below). Then hand-edit the generated `index.html` to taste.
3. **Want the parts labelled (spec sheet / teardown)?** → `inspect` the GLB first to learn its parts, then `stage --preset spec` (annotated, still model) or `--preset teardown` (exploded, per-part labels). Multi-part GLBs only.
4. **No model yet?** → Treat generation as Labs/input-source: `stereoframe gen "<prompt>"` writes a candidate textured GLB into `assets/` plus a `<name>.gen.json` provenance sidecar. **For better fidelity / reliable orientation, go image-first**: `gen --image front.png`, `gen --images front,side,back`, or `gen "<prompt>" --via-image`. **For alternative engines**, route through fal.ai: `gen "<prompt>" --provider fal --fal-model <id>`. All gen paths can output flawed single welded meshes; evaluate them before using them as hero assets.
5. **Want a cinematic multi-shot film (a directed spot, not one move)?** → write a **storyboard plan** (JSON shot list) and `stereoframe storyboard plan.json [--render]`. Use this after the asset is accepted; don't try to hide generated-model defects with cinematic polish.
6. **Just a sentence of direction (no JSON)?** → `stereoframe brief "<brief>" --model <glb> --render` calls an LLM to write the rich plan.json for you (`--llm-provider openai|anthropic`). Whole prompt→image→3D→film is available for experiments, but it is not the main promise.
7. **Custom scene / full control?** → hand-author `sf-scene` markup (canonical composition below).
...
Presets: `reveal` (dramatic dark spiral-in + rim light), `hero-orbit` (clean studio orbit), `turntable` (asset on a pedestal), `exploded-view` (parts separate outward — only for multi-component models with separate part meshes; single-mesh/rigged models won't separate), `spec` (auto-annotated asset preview), `teardown` (exploded labelled breakdown), `cinematic` (multi-shot reveal → macro → hero preview). The model is auto-framed (`<sf-model fit="2.6">` normalizes its size/center), so the preset's camera move + lighting rig + timing + finish frame ANY model consistently — no per-model camera/scale tweaking. Then hand-edit the generated `index.html` to taste.

**`spec` is the fully-automatic annotated preview**: `stage` runs `inspect` on the GLB, then writes a grounded, still-model, slow-arc scene with named spec callouts drawn onto its top parts (by triangle count), labeled by name + material character, fanned so they don't overlap, staggered through the back half. One command (`stereoframe stage product.glb --preset spec`) → an annotated evidence/preview scene. Multi-part GLBs only (it warns and renders un-annotated for single-mesh models). The model is kept still (not spun) so the tracked labels stay readable.
...
When a user gives you an accepted model and wants a standard preview, spec sheet, teardown, or explainable render, reach for `stage`. When the model is generated or untrusted, run `evaluate` first.
...
- **Per-part motion** — `turntable`/`sway`/`float`/`move` accept `part="<name|index>"` to animate ONE component (e.g. `<sf-animate target="#car" part="wheel" verb="turntable" axis="x">` = spinning wheel). With `core="ir"` you can instead **address the part directly** as `target="#car/wheel"` (model id `/` part name-or-index) — the preferred form; `part="…"` still works and lowers identically. A per-part `turntable` spins about the **part's own center**, so wheels spin in place even when the GLB parents them at the body origin (pivot not at the hub) — works regardless of how the model was rigged. Multi-part GLB only. **AI gen models (Meshy text/image-to-3D) are a single welded mesh** — `stereoframe segment <glb>` reports if a model is splittable; for real per-part motion (spinning wheels etc.) you need a genuinely multi-part source GLB (DCC/kitbash/CAD), not an AI-generated one.
- **`<sf-callout>`** (`target="#m" part="2" value="48MP" text="Main camera" anchor="left" start="1.2"`) — a leader line + typographic label that tracks a 3D part as the camera moves. Crisp DOM type, deterministic, hidden when off-screen. The strongest "this is an explainable asset preview, not a raw viewer" signal — pair it with `isolate` on the same `part` for a feature-highlight beat.

The `stage` presets already apply ground + light-sweep (and `spec`/`teardown` add the auto-callouts), so a plain `stage product.glb` gives a grounded, swept preview out of the box after the asset has passed evaluation.
Use `stage --preset cinematic` when the request needs an immediate directed result with multiple beats instead of one continuous move.
