...
| `light-sweep` | 0 | rotate the environment (reflections) `light-sweep` turns over the whole timeline — a slow specular sweep travels across metal/glass, useful for standardized previews and polished accepted-asset renders. Needs `environment` (`room`/`studio`/HDRI). Try 0.1 |
