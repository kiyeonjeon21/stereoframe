...
- **Releasing is lock-step.** `bun run release [patch|minor|major]` runs the full check suite, bumps both packages to the same version, commits/tags/pushes, then `release.yml` re-checks the tagged commit, publishes with `npm publish` (not `bun publish` — bun doesn't set the registry `readme` field), and creates a GitHub Release with generated notes. See `VERSIONING.md`.
